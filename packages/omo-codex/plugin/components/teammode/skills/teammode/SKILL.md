---
name: teammode
description: "Codex-only team orchestration for creating, running, coordinating, inspecting, archiving, or deleting teams of agents/threads/sessions, or working in parallel as a team. MUST USE for those requests. FIRST inspect the active tool surface (including tool_search for deferred tools), announce the route, then choose native MultiAgentV2 flat spawn_agent with task_name and fork_turns=\"none\", Codex App threads fallback, or plain subagents only when neither transport exists. The main session leads; members own concrete parts or perspectives, and the bundled script persists .omo/teams state plus member guidance. Use teams when coordination helps; use plain subagents for perfectly isolated work or ambiguity. Triggers: team mode, teammode, make/run as a team, team of agents, coordinate threads, parallel Codex threads, archive team."
---
