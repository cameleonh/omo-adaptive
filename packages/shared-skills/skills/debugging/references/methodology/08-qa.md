...
|---|---|
| **CLI tool** | Run the actual command end-to-end and capture relevant output and side effects. Use `tmux` only for interactive or long-running sessions that need it. |
...
## The silent-failure check (when relevant)

Audit these patterns when the original symptom was a silent failure or the changed path converts failures into success-shaped output. Do not scan adjacent unrelated code merely because this checklist exists.
